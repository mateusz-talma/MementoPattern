/*
Author: https://github.com/haarcuba/cpp-text-table
*/
...
private:
    ...
    long unsigned columns() const {
        return _rows[0].size();
    }

    unsigned long glyphLength(std::string s) const {
        size_t _byteLength = s.length();
#ifdef TEXTTABLE_ENCODE_MULTIBYTE_STRINGS
    ...
    void determineWidths() const {
        ...
        for (auto rowIterator = _rows.begin(); rowIterator != _rows.end(); ++rowIterator) {
            ...
            for (unsigned i = 0; i < row.size(); ++i) {
                _width[i] = _width[i] > glyphLength(row[i]) ? _width[i] : static_cast<unsigned int>(glyphLength(row[i]));
            }
        }
    }
